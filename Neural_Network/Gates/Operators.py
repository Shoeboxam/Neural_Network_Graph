...
class Add(Gate):
    def __call__(self, stimulus, parent=None):
        features = np.sum([child(stimulus, self) for child in self.children])
...
class Matmul(Gate):
    ...
    # Forward pass
    def __call__(self, stimulus, parent=None):
        features = np.matmul(self.children[0](stimulus, self), self.children[1](stimulus, self))
    ...
    def gradient(self, stimulus, variable, grad):
        if variable is self.children[0]:
            ...
            if variable in self.children[0].variables:
                grad = self.children[1](self, stimulus)
            else:
                grad = self.children[0](self, stimulus)
